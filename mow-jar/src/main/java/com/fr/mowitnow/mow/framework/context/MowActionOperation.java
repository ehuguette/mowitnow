...
/**
 * 
 * Implements mow actions.
 * 
 * @author ehuguette
 * @since 1.0
 */
public class MowActionOperation {
     ...
     */
    public EnumDirection next(final EnumDirection enumDirection) {
     ...
     */
    public EnumDirection previous(final EnumDirection enumDirection) {
	EnumDirection result = null;
	switch (enumDirection) {
	case N:
	    result = EnumDirection.E;
     ...
     */
    public Coordinates moveOneStep(final Coordinates coordinates, final EnumDirection direction) {
	Coordinates result = null;
	switch (direction) {
	case N:
	    result = new Coordinates(coordinates.getX(), increment(coordinates.getY()));
	    break;
	case E:
	    result = new Coordinates(decrement(coordinates.getX()), coordinates.getY());
	    break;
	case S:
	    result = new Coordinates(coordinates.getX(), decrement(coordinates.getY()));
	    break;
	case W:
	    result = new Coordinates(increment(coordinates.getX()), coordinates.getY());
	    break;
	default:
     ...
     */
    public boolean isInFieldCoordinates(final MowField mowField, final Coordinates mowCoordinates) {
	...
	if (limitTopRightField.getX() >= mowCoordinates.getX() && limitTopRightField.getY() >= mowCoordinates.getY()
     ...
     * @return Integer
     */
    protected Integer increment(final Integer value) {
	return value + 1;
    }
     ...
     * @return Integer
     */
    protected Integer decrement(final Integer value) {
	return value - 1;
    }
